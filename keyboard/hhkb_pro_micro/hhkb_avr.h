...
/*
 * For TMK HHKB alt controller(ATMega32U4)
 * changes port PD7 to PF0 and PB7 to PF1 and removes the code for the PD* pins
 *
 * row:     PB0-2           ->PB1-3
 * col:     PB3-5,6         ->PB4-6,PF6
 * key:     PD7(pull-uped)
 * prev:    PB7             ->PF7
 * power:   PD4(L:off/H:on)
 * row-ext: PC6,7 for HHKB JP(active low)
...
//    DDRB = 0xFF; PORTB = 0x40;          // change pins output
    DDRB |= 0x7E; PORTB &= 0x81;          // change pins output, PB1-3,4-6
    DDRF |= (1<<7); PORTF &= ~(1<<7);     // change pins output, PF7
    DDRF |= (1<<6); PORTF |=  (1<<6);     // change pins output, PF6
...
//    DDRB = 0x00; PORTB = 0xFF;          // change pins input with pull-up
    DDRB &=  0x81; PORTB |= 0x7E;          // change pins input with pull-up, PB1-3,4-6
    DDRF &= ~(1<<6); PORTF |=  (1<<6);     // change pins input with pull-up, PF6
    DDRF &= ~(1<<7); PORTF |=  (1<<7);     // change pins input with pull-up, PF7
...
{
    PORTB = (PORTB & 0x80) | (((COL) & 0x07)<<4) | (((ROW) & 0x07)<<1) | (PORTB & 0x01);
#ifdef HHKB_JP
